...
export class LoadComponent implements OnInit, OnDestroy {
    ...
    initialize() {
        ...
        const network = coininfo('rutanio').toBitcoinJS();
    ...
    downloadAndUnpack() {
        ...
        this.resetDone = false;
        this.noSpace = false;
        this.electronService.ipcRenderer.sendSync('reset-database');

        if (this.freeSpace > 3221225472) {
            // If user does "Copy as path" we must ensure we replace the quotes.
            const url = 'https://s3.amazonaws.com/rutanio.blockchain/rutanio-main.zip';
    ...
    unpack(source: string) {
        // Send array of path information to be used in path.join to get native full path in the main process.
        const pathInfo = [this.appState.daemon.datafolder, 'rutanio', 'RutanioMain'];
